...
// POST /api/import-custom-names - Import custom boost names (admin only)
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Import custom names data received:', {
      customNamesCount: validatedData.boostCustomNames.length
    })
    ...
    if (validatedData.boostCustomNames.length > 0) {
    ...
    return NextResponse.json({ message: 'Custom boost names imported successfully' })
