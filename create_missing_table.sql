...
CREATE POLICY "Users can view their own GP guide results"
  ...
  USING (auth.uid() IN (SELECT user_id FROM user_gp_guides WHERE id = user_gp_guide_results.gp_guide_id));

CREATE POLICY "Users can create their own GP guide results"
  ON user_gp_guide_results FOR INSERT
  WITH CHECK (auth.uid() IN (SELECT user_id FROM user_gp_guides WHERE id = user_gp_guide_results.gp_guide_id));
