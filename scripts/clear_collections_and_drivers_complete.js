...
async function clearCollectionsAndDriversComplete() {
  try {
    console.log('🧹 Clearing collections and drivers tables (with foreign key handling)...');
    
    // Clear user data first (foreign key constraints)
    console.log('🗑️  Clearing user data...');
    
    // Clear user_track_guides first (has foreign keys to drivers)
    const { error: userTrackGuidesError } = await supabase
      .from('user_track_guides')
      .delete()
      .neq('id', '00000000-0000-0000-0000-000000000000'); // Delete all

    if (userTrackGuidesError) {
      console.log('❌ User track guides delete error:', userTrackGuidesError.message);
    } else {
      console.log('✅ Cleared user_track_guides table');
    }
    ...
    // Clear user assets (if it exists)
    try {
      ...
      console.log('⚠️  User assets table not found, skipping...');
    }
    ...
    // Clear track guides (if it exists)
    try {
      ...
      console.log('⚠️  Track guides table not found, skipping...');
    }
    ...
    console.log('   - user_track_guides: ✅ Cleared');
    console.log('   - user_assets: ✅ Cleared (if exists)');
    console.log('   - track_guides: ✅ Cleared (if exists)');
...
clearCollectionsAndDriversComplete();